...
export type AgentPlanProps = {
  className?: string;
  /** When false, the card skips its own fade/slide-in (e.g. parent handles entrance). Default true. */
  animateEntrance?: boolean;
  /** When provided, the component is controlled — external tasks replace internal state. */
  tasks?: AgentPlanTask[];
};
...
export default function AgentPlan({
  ...
  const [internalTasks, setTasks] = useState<AgentPlanTask[]>(initialTasks);
  const tasks = propTasks ?? internalTasks;

  // Auto-expand the first in-progress task when driven externally.
  const [expandedTasks, setExpandedTasks] = useState<string[]>(["1"]);
  useEffect(() => {
    if (!propTasks) return;
    const activeId = propTasks.find((t) => t.status === "in-progress")?.id;
    if (activeId) setExpandedTasks([activeId]);
  }, [propTasks]);
                        ...
                        <AnimatePresence mode="wait">
                          ...
                          >
                            {task.status === "completed" ? (
                              ...
                              <CircleDotDashed className="h-4 w-4 animate-spin text-blue-500" style={{ animationDuration: "1.4s" }} />
                                      ...
                                      <AnimatePresence mode="wait">
                                        ...
                                        >
                                          {subtask.status === "completed" ? (
                                            ...
                                            <CircleDotDashed className="h-3.5 w-3.5 animate-spin text-blue-500" style={{ animationDuration: "1.4s" }} />
